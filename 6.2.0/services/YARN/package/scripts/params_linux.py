...
hadoop_mapr_home = "/usr/lib/hadoop-mapreduce"
...
if version is not None:
  hadoop_mapr_versioned_home = format("{stack_root}/hadoop-mapreduce")
  hadoop_yarn_versioned_home = format("{stack_root}/hadoop-yarn")
...
yarn_atsv2_hbase_versioned_home = format("{stack_root}/hbase")
...
yarn_hbase_app_hdfs_path = format("/hadoop/apps/hbase")
yarn_service_app_hdfs_path = format("/hadoop/apps/yarn")
...
yarn_service_dep_source_path = format("{stack_root}/hadoop-yarn/lib/service-dep.tar.gz")
yarn_hbase_user_version_path = format("{yarn_hbase_user}")
...
yarn_timeline_jar_location = format("file://{stack_root}/hadoop-yarn/timelineservice/{coprocessor_jar_name}")
...
yarn_hbase_table_create_cmd = format("export HBASE_CLASSPATH_PREFIX={stack_root}/hadoop-yarn/timelineservice/*;{yarn_hbase_kinit_cmd} {hbase_cmd} {class_name}")
